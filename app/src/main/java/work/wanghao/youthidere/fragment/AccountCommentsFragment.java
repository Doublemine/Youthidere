...
import work.wanghao.youthidere.adapter.AccountCommentsAdapter;
 ...
 */
public class AccountCommentsFragment extends Fragment implements SwipeRefreshLayout.OnRefreshListener {
    ...
    private View mView;
    private AccountCommentsAdapter adapter;
    private List<CompleteItemComments> mData;
    private Token token;
    private AccountInfoFragmentCallback mCallBack;
    private static AccountCommentsFragment mFragment;
    
    public  static AccountCommentsFragment getInstance(){
        if(mFragment==null){
            mFragment=new AccountCommentsFragment();
        }
        return mFragment;
    }
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        adapter=new AccountCommentsAdapter(getActivity());
        mData=new ArrayList<CompleteItemComments>();
    ...
    private void getDataByDate(Token token){
        ...
        AsyncTask<String,Void,List<CompleteItemComments>> initTask=new AsyncTask<String, Void, List<CompleteItemComments>>() {
            @Override
            protected void onPostExecute(List<CompleteItemComments> readHistoryItems) {
                if(readHistoryItems==null){
                    Snackbar.make(mView,"发生网络错误无法获取用户的评论数据!",Snackbar.LENGTH_SHORT).show();
                    mRefresh.setRefreshing(false);
                    return;
                }else {
                    if(readHistoryItems.size()<=0){
                        Snackbar.make(mView,"您还没有发表任何评论",Snackbar.LENGTH_SHORT).show();
                    }
                    mData.clear();
            ...
            @Override
            protected List<CompleteItemComments> doInBackground(String... params) {
               if(NetUtils.isNetConnect()){
                   return HttpUtils.getAccountCommentsFromServer(1,params[0]);
               }else {
                   return null;
               }
            }
